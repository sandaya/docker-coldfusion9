...
ENV REFRESHED_AT 2018_01_09_4

RUN apt-get update
RUN apt-get install -y sudo wget unzip xsltproc apache2 default-jre && apt-get clean
...
# Replace solr startup script. fixed bug to add "" on SUCMD
RUN mv /tmp/cfsolr /opt/coldfusion9/solr/cfsolr
